...
namespace Storm
{
    ...
    struct Point
    {
        ...
        {
            return compareValue(point_[0], other.point_[0])
                && compareValue(point_[1], other.point_[1]);
        }
    ...
    struct Size
    {
        ...
        bool isZero() const
        {
            return Storm::isZero(size_[0])
                && Storm::isZero(size_[1]);
        }

        bool isValid() const
        {
            return size_[0] >= ValueType()
                && size_[1] >= ValueType();
        }

        bool operator == (const Size& other) const
        {
            return compareValue(size_[0], other.width())
                && compareValue(size_[1], other.height());
        }
        bool operator != (const Size&  other) const
        {
            return !compareValue(size_[0], other.width())
        ...
        bool operator < (const Size&  other) const
        {
            return size_[0] < other.width()
                || size_[1] < other.height();
        }
    ...
    {
        os << "{" << "width:" << sz.w   () << ",height:" << sz.height() << "}" << std::endl;
        return os;
    }
    ...
    struct Rect
    {
        ...
        {
            return compareValue(rect_[0], other.rect_[0])
                && compareValue(rect_[1], other.rect_[1])
                && compareValue(rect_[2], other.rect_[2])
                && compareValue(rect_[3], other.rect_[3]);
        }
