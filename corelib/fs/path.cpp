...
namespace Storm
{
    ...
    {
        return std::filesystem::path(fullPath_).parent_path().wstring();
    }

    Path Path::rootPath() const
    {
        return std::filesystem::path(fullPath_).root_path().wstring();
    }

    std::wstring Path::fileName() const
    {
        return std::filesystem::path(fullPath_).filename().wstring();
    }
